...
class LinePredictor(PosePredictor):
    def __init__(
        ...
        # Dont use Lines (for testint the difference)
        self.dont_refine_ransac = debug_dont_refine_ransac

    
    def remove_short_line_segments(self,line_segs_2d:np.ndarray) -> np.ndarray:
        """
        ...
        return line_segs_2d[lengths > (self.min_line_length_px**2)]
    ...
    def line_segment_regression(self,xyz_points:np.ndarray)->np.ndarray|None:
        """
    ...
    def visualize_features_2d(
        ...
        lines_xy1 = [((line[0], line[1]), (line[2], line[3])) for line in lines1]
        lc1 = LineCollection(lines_xy1, linewidths=1, alpha=0.4)
        axes[0, 0].imshow(cv2.cvtColor(img1, cv2.COLOR_BGR2RGB))
        axes[0, 0].add_collection(lc1)
        axes[0, 0].set_title("LSD Lines — Camera 1")
        ...
        axes[0, 0].add_collection(lc1m)


        lines_xy2 = [((line[0], line[1]), (line[2], line[3])) for line in lines2]
        lc2 = LineCollection(lines_xy2, linewidths=1, alpha=0.4)
        axes[0, 1].imshow(cv2.cvtColor(img2, cv2.COLOR_BGR2RGB))
        axes[0, 1].add_collection(lc2)
        axes[0, 1].set_title("LSD Lines — Camera 2")
    ...
    def est_base_t_cam2(self,
        ...
        lines_img1 = lsd.detect(cv2.cvtColor(cam1_bgr_image, cv2.COLOR_BGR2GRAY))[0].squeeze(1)
        lines_img2 = lsd.detect(cv2.cvtColor(cam2_bgr_image, cv2.COLOR_BGR2GRAY))[0].squeeze(1)

        lines_img1 = self.remove_short_line_segments(lines_img1)
        lines_img2 = self.remove_short_line_segments(lines_img2)
        ...
        for line_pair in line_pairs:
            line_segment_3d = self.line_segment_regression(self.line_seg_2d_to_3d_points(line_pair[0], base_xyz_image))
...
if __name__ == "__main__":
    ...
    predictor = LinePredictor(data.headset_intrinsics, extract_and_match=ExtractAndMatchLoMa(loma_variant="LoMaB"), debug_dont_refine_ransac=False)
