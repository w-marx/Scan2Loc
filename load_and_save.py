...
def load_input_data(input_folder:str) -> dict[str, np.ndarray | None | list[str]]:
    ...
    :return: A Dictionary:
    {
        ...
        robot_cam_mtx: 3x3 numpy array of the cam_mtx or None (either headset_calibration_images or robot_cam_mtx is not none)
        robot_images: robot_images (NxWxHx3-uint8 numpy array),
        robot_images_names: robot_image_names list of strings of length number of robot images,
        robot_calibration_images: robot_calibration_images (NxWxHx3-uint8 numpy array)
    }
    """
    ...
    if os.path.exists(f"{input_folder}/headset.vrs"):
        ...
        headset_images, _ = get_images(f"{input_folder}/headset_calibration")
    ...
    # Load Robot images
    robot_images, robot_image_names = get_images(f"{input_folder}/robot")

    # Load Robot calibration images
    robot_calibration_images, _ = get_images(f"{input_folder}/robot_calibration")
    ...
    ret_dict = {
        ...
        "robot_calibration_images":robot_calibration_images,
    }

    return ret_dict


def save_cam_properties_as_json(output_folder:str, output_file_name:str, cam_mtx:np.ndarray, cam_dist_coef:np.ndarray):
    """
    ...
    :return:
    """
    headset_camera_data = {'camera_matrix': cam_mtx.tolist(), 'dist_coef': cam_dist_coef.tolist(),}
...
def save_output_data(
        ...
        headset_cam_dist_coef:np.ndarray,
        ...
        robot_cam_dist_coef:np.ndarray,
    ...
    save_cam_properties_as_json(output_folder, "headset_cam_calibration", headset_cam_mtx, headset_cam_dist_coef)
    save_cam_properties_as_json(output_folder, "robot_cam_calibration", robot_cam_mtx, robot_cam_dist_coef)
