...
class FastRayIntersectionError:
    ...
    def compute_ray_intersection_error_img(self, base_t_cam1:np.ndarray, base_t_cam2:np.ndarray, dim:tuple[int, int], size:int = 5, stride:int = 1)->np.ndarray:
        """
        ...
        :param stride: will divide each image dimension by stride while still sampling the whole img coords every stride pixels
        ...
        intersection_diff = np.linalg.norm(locations_m1-locations_m2, axis=-1).reshape(len(rows),len(cols))
